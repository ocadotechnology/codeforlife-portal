...
import { createClass, createStudent, deleteClass, deleteAllStudents, goToClassSettings, editClassSettings, transferClass } from "../support/classTester.ts"
...
describe("Teacher", () => {
  ...
  it("can delete a class only when empty", () => {
    ...
    cy.get('#messages').should('contain.text', `This class still has students, please remove or delete them all before deleting the class.`);
  ...
  it("can edit a class", () => {
    ...
    cy.get('.container').should('contain.text', `This class is not currently accepting external requests.`);
    ...
    cy.get('.container').should('contain.text', `This class is currently set to always accept requests.`);
