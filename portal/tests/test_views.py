...
from game.models import Level, LevelMetrics
...
class TestViews(TestCase):
    ...
    def test_student_dashboard_view(self):
        ...
        LevelMetrics.objects.create(level=level1001, student=student, top_score=20)
        LevelMetrics.objects.create(level=level1004, student=student, top_score=10)
        ...
        LevelMetrics.objects.create(level=custom_level2, student=student, top_score=10)
...
class TestUser(CronTestCase):
    ...
    def send_inactivity_reminder(
        ...
        self.student_user.date_joined = timezone.now() - timedelta(days=days, hours=12)
        self.student_user.save()
        self.indy_user.last_login = timezone.now() - timedelta(days=days, hours=12)
        self.indy_user.save()
    ...
    def test_second_inactivity_reminder_view(self):
        ...
        self.send_inactivity_reminder(
            ...
            True,
            "inactive_users_on_website_second_reminder",
            personalization_values={"DAYS_LEFT": USER_RETENTION_PERIOD - USER_2ND_INACTIVE_REMINDER_DAYS},
        )
        self.send_inactivity_reminder(
            974,
            "second-inactivity-reminder",
