...
class TeacherSignupForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if email and Teacher.objects.filter(user__email=email).exists():
...
class TeacherEditAccountForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if email:
            ...
            if ((len(teachers) == 1 and teachers[0].user != self.user) or
                    len(teachers) > 1):
                raise forms.ValidationError("That email address is already in use")
...
class ClassMoveForm(forms.Form):
    ...
    def __init__(self, teachers, *args, **kwargs):
        ...
        for teacher in teachers:
            teacher_choices.append((teacher.id, teacher.user.first_name + ' '
...
class TeacherAddExternalStudentForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if Student.objects.filter(
                class_field=self.klass, user__first_name__iexact=name).exists():
            raise forms.ValidationError("There is already a student called '" + name
                                        + "' in this class")
    ...
    def __init__(self, classes, *args, **kwargs):
        ...
        for klass in classes:
            class_choices.append((klass.id, klass.name + ' (' + klass.access_code + '), '
                                  + klass.teacher.user.first_name + ' '
                                  + klass.teacher.user.last_name))
...
def validateStudentNames(klass, names):
    ...
    if klass:
        ...
        clashes_found = []
        for name in names:
            if (students.filter(user__first_name__iexact=name).exists() and
