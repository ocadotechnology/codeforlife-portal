...
from portal.forms.registration_new import TeacherPasswordResetForm, PasswordResetSetPasswordForm, \
    StudentPasswordResetForm
from portal.permissions import not_logged_in, not_fully_logged_in
from portal.helpers.emails_new import PASSWORD_RESET_EMAIL
from portal import app_settings

recaptcha_client = RecaptchaClient(app_settings.RECAPTCHA_PRIVATE_KEY, app_settings.RECAPTCHA_PUBLIC_KEY)


@user_passes_test(not_logged_in, login_url=reverse_lazy('login_new'))
def student_password_reset(request):
    ...
    return password_reset(request, usertype, from_email=PASSWORD_RESET_EMAIL, template_name='redesign/reset_password_student.html',
                          password_reset_form=form, is_admin_site=True)


@user_passes_test(not_fully_logged_in, login_url=reverse_lazy('login_new'))
def teacher_password_reset(request):
    ...
    return password_reset(request, usertype, from_email=PASSWORD_RESET_EMAIL, template_name='redesign/reset_password_teach.html',
                          password_reset_form=form, is_admin_site=True)
...
@csrf_protect
def password_reset(request, usertype, is_admin_site=False, template_name='redesign/reset_password_teach.html',
                   email_template_name='redesign/reset_password_email.html',
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, 'redesign/reset_password_email_sent.html', {'usertype': usertype})
    else:
...
def password_reset_done(request, template_name='redesign/reset_password_email_sent.html', current_app=None, extra_context=None):
    context = {
...
def password_reset_confirm(request, usertype, uidb64=None, token=None,
                           template_name='redesign/reset_password_confirm.html',
    ...
    if user_is_authenticated(user, token_generator, token):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                form.save()
                return render(request, 'redesign/reset_password_done.html', {'usertype': usertype})
        else:
