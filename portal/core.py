...
class LoginView(SuccessURLAllowedHostsMixin, IdempotentSessionWizardView):
    ...
    def done(self, form_list, **kwargs):
        ...
        if device:
            ...
            if (
                getattr(settings, "TWO_FACTOR_REMEMBER_COOKIE_AGE", None)
                and "remember" in form_obj.fields
                and form_obj["remember"].value()
            ):
    ...
    def get_context_data(self, form, **kwargs):
        ...
        if self.steps.current == "token":
            ...
            context["other_devices"] = [
                phone
                for phone in backup_phones(self.get_user())
